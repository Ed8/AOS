...
if [ -e /var/www/$username/public_html/index.php ]; then
	...
	sudo echo "Bienvenue sur $username.$domain !" >> sudo /var/www/$username/public_html/index.php
...
if [ -e /etc/apache2/sites-available/$username.$domain.conf ]; then
	...
	sudo echo "<VirtualHost *:80>" >> sudo /etc/apache2/sites-available/$username.$domain.conf
	sudo echo "	ServerName $username.$domain" >> sudo /etc/apache2/sites-available/$username.$domain.conf
	sudo echo "	Documentroot /var/www/$username/public_html" >> sudo /etc/apache2/sites-available/$username.$domain.conf
	sudo echo "</VirtualHost>" >> sudo /etc/apache2/sites-available/$username.$domain.conf
