...
if [ -e /var/www/$username/public_html/index.php ]; then
	...
	sudo echo "Bienvenue sur $username.$domain !" | sudo tee /var/www/$username/public_html/index.php
...
if [ -e /etc/apache2/sites-available/$username.$domain.conf ]; then
	...
	sudo echo "<VirtualHost *:80>" | sudo tee /etc/apache2/sites-available/$username.$domain.conf
	sudo echo "	ServerName $username.$domain" | sudo tee -a /etc/apache2/sites-available/$username.$domain.conf
	sudo echo "	Documentroot /var/www/$username/public_html" | sudo tee -a /etc/apache2/sites-available/$username.$domain.conf
	sudo echo "</VirtualHost>" | sudo tee -a /etc/apache2/sites-available/$username.$domain.conf
...
sudo bash addenregzone.sh $username $domain fqdn 88.177.168.133 
