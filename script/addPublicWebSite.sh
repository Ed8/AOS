...
if [ -d /var/www/$username/$fqdn.${domain}_public_html ]; then
	...
	sudo mkdir /var/www/$username/$fqdn.${domain}_public_html
	sudo chown $username /var/www/$username/$fqdn.${domain}_public_html
	sudo echo "Repository /var/www/$username/$fqdn.$domain_public_html created !"
fi

#Creating public web page for public_html
if [ -e /var/www/$username/$fqdn.${domain}_public_html/index.php ]; then
	sudo echo "File /$username/$fqdn.$domain_public_html/index.php already exist !"
else
	sudo touch /var/www/$username/$fqdn.${domain}_public_html/index.php
	sudo echo "Bienvenue sur $fqdn.$domain !" | sudo tee /var/www/$username/$fqdn.${domain}_public_html/index.php
	sudo chown $username /var/www/$username/$fqdn.${domain}_public_html/index.php
...
if [ -e /etc/apache2/sites-available/$fqdn.$domain.conf ]; then
	...
	sudo echo "	Documentroot /var/www/$username/$fqdn.${domain}_public_html" | sudo tee -a /etc/apache2/sites-available/$fqdn.$domain.conf
