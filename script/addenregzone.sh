	...
	if [ -e "/etc/tinydns/root/$nomuser.zone" ]; then
		if sudo grep -q @$nomuser.$domaine /etc/tinydns/root/$nomuser.zone; then
			...
			if [ $adresse = "88.177.168.133" ]; then
				sudo echo "@$nomuser.$domaine:$adresse:mx:86400" >> /etc/tinydns/root/$nomuser.zone	
	...
	if [ -e "/etc/tinydns/root/$nomuser.zone" ]; then
		if sudo grep -q +www.$nomuser.$domaine /etc/tinydns/root/$nomuser.zone; then
			sudo echo "Ce web existe deja!"
 	   		...
 	   		sudo echo "+www.$nomuser.$domaine:$adresse:a:86400 " >> /etc/tinydns/root/$nomuser.zone
			sudo echo "+www.dev.$nomuser.$domaine:$adresse:a:86400 " >> /etc/tinydns/root/$nomuser.zone
		fi

	elif sudo grep -q "+www.$nomuser" "/etc/tinydns/root/aos.zone"; then
  	   	sudo echo "Le web existe deja!"
	else
  	 	sudo echo "+www.$nomuser.$domaine:88.177.168.133:a:86400 " >> /etc/tinydns/root/aos.zone
		sudo echo "+www.dev.$nomuser.$domaine:88.177.168.133:a:86400 " >> /etc/tinydns/root/aos.zone
	fi
fi
